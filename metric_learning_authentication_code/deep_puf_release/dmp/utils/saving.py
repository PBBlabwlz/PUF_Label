...
class SaverMixin:
    ...
    def save_img_sequence(
        ...
        if name and self._wandb_logger:
            dmp.warn("Wandb logger does not support video logging yet!")
        return save_path
    ...
    def save_img_sequences(
        ...
        for f in os.listdir(seq_dir_):
            ...
            try:
                ...
                dmp.warn(f"Video saving for directory {seq_dir_} failed!")
...
#----------------------plot----------------------
    ...
    def evaluate(self, sheet_dict, y_true_dict, y_pred_dict, save_subdir=None):
        for key in sheet_dict.keys():
            self.get_accuracy_roc(y_pred_dict[key], y_true_dict[key])
            heat = np.array(sheet_dict[key])
            self.plot_results(heat, y_true_dict[key], y_pred_dict[key], key, save_subdir)
            self.save_csv(heat, key, save_subdir)
    
    def save_csv(self, heat, key, save_subdir):
        save_image_dir = os.path.join(self.get_save_dir(), save_subdir)
        os.makedirs(save_image_dir, exist_ok=True)
        np.savetxt(os.path.join(save_image_dir, f'{key}.csv'), heat, delimiter=',', fmt='%f')

    def plot_results(self, heat, y_true, y_pred, key_name, save_subdir):
        save_image_dir = os.path.join(self.get_save_dir(), save_subdir)
        os.makedirs(save_image_dir, exist_ok=True)
        self.make_heatmap(heat, os.path.join(save_image_dir, f'heat_{key_name}.jpg'), y_pred, y_true)
        self.make_heatmap(heat, os.path.join(save_image_dir, f'heat_{key_name}_minmax.jpg'), y_pred, y_true,
                          bar_mode='min_max')

        index1 = [i for i, y in enumerate(y_true) if y == 1]
        index2 = [i for i, y in enumerate(y_true) if y == 0]
        self.plot_histograms(y_pred, index1, index2, key_name, save_image_dir)
        ...
        # record these two values and its distance in a txt file
        with open(os.path.join(save_image_dir, f'{key_name}_max_min.txt'), 'w') as f:
            f.write(f"Max value of y_pred for index1: {max_val_index2}\n")
            f.write(f"Min value of y_pred for index2: {min_val_index1}\n")
            f.write(f"Distance: {- max_val_index2 + min_val_index1}\n")
    ...
    def plot_histograms(self, y_pred, index1, index2, key_name, save_image_dir):
        x1 = [y_pred[i] for i in index1]
        x2 = [y_pred[i] for i in index2]
        x1 = np.array(x1)
        x2 = np.array(x2)
        results_path = os.path.join(save_image_dir, f'hist_{key_name}.png')
